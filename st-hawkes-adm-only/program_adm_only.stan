...
functions {
  ...
  vector lambda_cond_multidim_dt_m(matrix space_dists_ragged, matrix Times_events_index_ragged, int[] Times_obs,
                                  matrix t_ind_ragged, int[] t_ind_times_ragged, real[] mu, real[] alpha, real[] beta,  real[] sigma,
    ...
    for (row_ind_y in 1:n_rows_total){
      y_m[row_ind_y] = mu[m] + R_m[row_ind_y];
    }
...
model {
  ...
  for (m in 1:M){
    ...
    L[m] = lambda_cond_multidim_dt_m(space_dists_ragged, Times_events_index_ragged, Times_obs, t_ind_m, t_ind_times_ragged,
                                  mu, alpha, beta, sigma, M, n_locations, T_obs, n_rows_total, m, n_events, n_events_all);
