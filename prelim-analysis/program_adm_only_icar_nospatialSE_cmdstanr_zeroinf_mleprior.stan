...
functions {
  ...
  vector lambda_cond_multidim_dt_m(matrix Times_events_index_ragged, array[] int Times_obs,
                                  matrix t_ind_ragged, matrix space_ind_ragged, array[] int t_ind_times_ragged, vector mu_vec, array[] real alpha, real beta,  
    ...
    row_vector[M] ones;
    matrix[n_locations,n_events_all] rbf_ones;
    ...
    for (l in 1:M){p = (m-1) * M + l;
      ...
      for (i in 1:n_locations){
        decay_dists_vals[i,pos_start:pos_end] = to_row_vector(col(Times_events_index_ragged[pos_start:pos_end],3)) .* space_ind_ragged[i,pos_start:pos_end];
      }
...
data {
  ...
  matrix<lower = 0>[n_locations,n_events_all_M] space_ind_ragged;
...
transformed parameters {
  ...
  jacobian += (-prop_0_logit - 2*log(1+exp(-prop_0_logit))) ; //for alpha, sigma and prop_0
}
...
model {
  ...
  for (m in 1:M){
    ...
    L[m] = lambda_cond_multidim_dt_m(Times_events_index_ragged, Times_obs, t_ind_m, space_ind_ragged, t_ind_times_ragged,
                                  mu_vec, alpha, beta, M, n_locations, T_obs, n_rows_total, m, n_events, n_events_all);
