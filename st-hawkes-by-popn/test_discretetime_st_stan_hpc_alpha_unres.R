...
sth_asia_st %<>%
  mutate(pop_index_1 = POPULATION_psk <= quantile(sth_asia_st$POPULATION_psk,probs=grid$pop_threshold),
         pop_index_2 = POPULATION_psk > quantile(sth_asia_st$POPULATION_psk,probs=grid$pop_threshold))
...
pop_index_1 = lapply(1:M,function(m) {pop=left_join(Times_all[[m]][Times_all[[m]]$times>Tinf,],(pop_index_data%>%filter(EVENT_TYPE %in% type[m])%>%select(times,centroids,pop_index_1)),
                                                by=c("times"="times","centroids"="centroids")); return(as.numeric(pop$pop_index_1))})
pop_index_2 = lapply(1:M,function(m) {pop=left_join(Times_all[[m]][Times_all[[m]]$times>Tinf,],(pop_index_data%>%filter(EVENT_TYPE %in% type[m])%>%select(times,centroids,pop_index_2)),
...
data$pop_index_1 = pop_index_1
data$pop_index_2 = pop_index_2
...
fit <- stan(file = "program_alpha_unres.stan",
            data = data,
            iter = grid$N_MCMC,
            warmup = grid$burn_in,
            thin = 1,
...
message("================================",ind)
saveRDS(fit, paste0("acled_st_hawkes_alpha_unres_",time_aggregation,"_pop_",ind,".rds"))
