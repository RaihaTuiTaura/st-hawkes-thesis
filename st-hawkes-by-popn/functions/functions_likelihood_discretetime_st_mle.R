...
lambda_cond_multidim_dt_st_m = function(theta,m,decay_fn,decay_space_fn,data){
  ...
  g=function(l){
    p = (m-1)*M+l
    decay_space_vals =  unlist(lapply(data$Times_obs[[m]], function(t){
    ...
    L_m = (h$alpha_1[p]*(data$pop_index_1[[m]]==1) + h$alpha_2[p]*(data$pop_index_2[[m]]==1))*decay_space_vals
    return(L_m)
  }
  ...
  R_m =rowSums(L_m)
  y_m=(mu_1[m]*(data$pop_index_1[[m]]==1) + mu_2[m]*(data$pop_index_2[[m]]==1)) + R_m
  return(y_m)
...
log_likelihood_multidim_dt_st=function(data,decay_fn,decay_space_fn,theta_vec,theta=NULL,op='sum'){
  ...
  theta = list(mu_1 = theta_vec[1:M],
               mu_2 = theta_vec[(M+1):(2*M)],
               h=list(alpha_1 = theta_vec[(2*M+1):(2*M+M^2)],
                      alpha_2 = theta_vec[(2*M+M^2+1):(2*M+2*M^2)],
                      beta = theta_vec[(2*M+2*M^2+1):(2*M+3*M^2)]),
               sigma = theta_vec[(2*M+3*M^2+1):(2*M+4*M^2)])
